...
class TestLifecycleRecovery(LifecycleFixture):
    ...
    def test_second_move_step_failure_preserves_partial_evidence_and_can_resume(self):
        ...
        final = restarted.recover("move-partial", mode="resume", approve_plan_id=plan["plan_id"])
...
"""
        for index, boundary in enumerate(boundaries):
            with self.subTest(boundary=boundary):
                ...
                restarted = Manager(self.root)
                self.addCleanup(restarted.repository.close)
                receipt = restarted.recover(transaction_id, mode="resume", approve_plan_id=plan["plan_id"])
                self.assertEqual(receipt["status"], "completed")
                self.assertEqual((target / "payload").read_text(), "H1")
...
"""
        ...
        restarted = Manager(self.root)
        self.addCleanup(restarted.repository.close)
        observed = restarted.verify(receipt["installation_id"])
