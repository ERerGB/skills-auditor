...
If the initial preflight finds any changed precondition, apply exits `3` before starting filesystem
writes. Preconditions are also rechecked per action: a later `stale_plan` can occur after earlier
actions completed. In either case, inspect the current state, then generate and review a new plan.
...
All preconditions across all roots are checked before the first write; each action's source, target,
and reserved archive are rechecked immediately before that action. Replacement and archive/link
actions attempt a local rollback when their link creation fails. Separate filesystem operations
cannot be globally atomic across roots, and earlier completed actions are not automatically undone
when a later action fails. Apply writes a failed receipt containing completed, verified actions and
the error when the filesystem still permits it.

A v1 receipt is a result record, not a write-ahead execution journal. An action is added to `results`
only after its post-operation checks pass: the failing action may already have changed the
filesystem even when `results` is empty. The receipt does not provide crash recovery or prove that
unlisted targets are unchanged. If receipt storage itself fails, the command reports that
explicitly. Preserve available evidence and audit every target and reserved archive in the plan
before preparing a retry.
...
its apply. A `noop` plan still requires this explicit apply to issue a new receipt for a changed
source version; planning alone does not renew approval.

The v1 `approval` object is a current version-match result, not a persisted revocation record.
Verification does not modify the receipt or remember past failures. If the original reviewed tree
and target state are restored, the same completed receipt can report `valid` again. Verification
does not restore files, roll back versions, or approve a different version.

Verification does not prove runtime behavior or semantic skill safety. Installed entries remain
live symlinks, so verification can detect a source change but cannot prevent a host from consuming
it between verification runs.
